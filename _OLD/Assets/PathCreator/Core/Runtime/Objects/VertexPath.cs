    ...
    public class VertexPath
    {
        ...
        public VertexPath(
            ...
            this(
                bezierPath,
                VertexPathUtility
                    .SplitBezierPathByAngleError(bezierPath,
                    maxAngleError,
                    minVertexDst,
                    VertexPath.accuracy),
                transform
        ...
        public VertexPath(
            ...
            this(
                bezierPath,
                VertexPathUtility
                    .SplitBezierPathEvenly(bezierPath,
        ...
        )
        {
            ...
            bounds =
                new Bounds((
                    pathSplitData.minMax.Min + pathSplitData.minMax.Max
                    ) /
                    2,
            ...
            up =
                (bounds.size.z > bounds.size.y) ? Vector3.up : -Vector3.forward;
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                cumulativeLengthAtEachVertex[i] =
                    pathSplitData.cumulativeLength[i];
                ...
                if (space == PathSpace.xyz)
                {
                    if (i == 0)
                    {
                        localNormals[0] =
                            Vector3
                                .Cross(lastRotationAxis,
                                pathSplitData.tangents[0])
                                .normalized;
                    }
                    else
                    {
                        ...
                        Vector3 r =
                            ...
                            2 /
                            sqrDst *
                            Vector3.Dot(offset, lastRotationAxis);
                        Vector3 t =
                            localTangents[i - 1] -
                            offset *
                            2 /
                            sqrDst *
                            Vector3.Dot(offset, localTangents[i - 1]);
                        ...
                        Vector3 n =
                            Vector3
                                .Cross(finalRot, localTangents[i])
                                .normalized;
                        localNormals[i] = n;
                ...
                {
                    localNormals[i] =
                        Vector3.Cross(localTangents[i], up) *
                        ((bezierPath.FlipNormals) ? 1 : -1);
                }
            }
            ...
            if (space == PathSpace.xyz && isClosedLoop)
            {
                ...
                float normalsAngleErrorAcrossJoin =
                    Vector3
                        .SignedAngle(localNormals[localNormals.Length - 1],
                        localNormals[0],
                        localTangents[0]);

                // Gradually rotate the normals along the path to ensure start and end normals line up correctly
                if (
                    Mathf.Abs(normalsAngleErrorAcrossJoin) > 0.1f // don't bother correcting if very nearly correct
                )
                {
                    for (int i = 1; i < localNormals.Length; i++)
                    {
                        ...
                        Quaternion rot =
                            Quaternion.AngleAxis(angle, localTangents[i]);
                        localNormals[i] =
                            rot *
                            localNormals[i] *
                            ((bezierPath.FlipNormals) ? -1 : 1);
            ...
            if (space == PathSpace.xyz)
            {
                ...
                )
                {
                    ...
                    int startVertIndex =
                        pathSplitData.anchorVertexMap[anchorIndex];
                    int endVertIndex =
                        pathSplitData.anchorVertexMap[anchorIndex + 1];
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        Quaternion rot =
                            Quaternion
                                .AngleAxis(angle, localTangents[vertIndex]);
                        localNormals[vertIndex] =
                            (rot * localNormals[vertIndex]) *
                            ((bezierPath.FlipNormals) ? -1 : 1);
                    }
                }
            }
        ...
        {
            return MathUtility
                .TransformDirection(localTangents[index], transform, space);
        }

        public Vector3 GetNormal(int index)
        {
            return MathUtility
                .TransformDirection(localNormals[index], transform, space);
        }

        public Vector3 GetPoint(int index)
        {
            return MathUtility
                .TransformPoint(localPoints[index], transform, space);
        }
        ...
        )
        {
            ...
            return Vector3
                .Lerp(GetPoint(data.previousIndex),
        ...
        )
        {
            ...
            Vector3 dir =
                Vector3
                    .Lerp(localTangents[data.previousIndex],
        ...
        )
        {
            ...
            Vector3 normal =
                Vector3
                    .Lerp(localNormals[data.previousIndex],
        ...
        )
        {
            ...
            Vector3 normal =
                Vector3
                    ...
                    data.percentBetweenIndices);
            return Quaternion
                .LookRotation(MathUtility
                    .TransformDirection(direction, transform, space),
        ...
        public float GetClosestDistanceAlongPath(Vector3 worldPoint)
        {
            TimeOnPathData data = CalculateClosestPointOnPathData(worldPoint);
            return Mathf
                .Lerp(cumulativeLengthAtEachVertex[data.previousIndex],
        ...
        TimeOnPathData CalculateClosestPointOnPathData(Vector3 worldPoint)
        {
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                Vector3 closestPointOnSegment =
                    MathUtility
                        .ClosestPointOnLineSegment(worldPoint,
                        GetPoint(i),
                        GetPoint(nextI));
                float sqrDst =
                    (worldPoint - closestPointOnSegment).sqrMagnitude;
                if (sqrDst < minSqrDst)
                {
                    minSqrDst = sqrDst;
            ...
            float closestSegmentLength =
                (
                GetPoint(closestSegmentIndexA) - GetPoint(closestSegmentIndexB)
                ).magnitude;
            float t =
                (closestPoint - GetPoint(closestSegmentIndexA)).magnitude /
                closestSegmentLength;
            return new TimeOnPathData(closestSegmentIndexA,
        ...
        public struct TimeOnPathData
        {
            ...
            public TimeOnPathData(
                int prev,
                int next,
                float percentBetweenIndices
            )
            {
