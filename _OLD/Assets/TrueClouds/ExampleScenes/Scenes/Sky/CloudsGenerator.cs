    ...
    void Start()
    {
        ...
        while (curPos.z < EndPos.z)
        {
            ...
            while (curPos.x < EndPos.x)
            {
                ...
                int x =
                    (int)(HeightMap.width * curPos.x / (EndPos - StartPos).x);
                int y =
                    (int)(HeightMap.height * curPos.z / (EndPos - StartPos).x);
                ...
                GameObject placed =
                    (GameObject)
                    Instantiate(Prefabs[id], curPos, Quaternion.identity);
                placed.transform.localScale = new Vector3(width, 10, width);
